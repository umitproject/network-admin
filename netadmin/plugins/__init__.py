...
"""
Plugins app consists of core elements such as Plugin class or load_plugins()
function, and set of tools that lets us extend Network Administrator: widgets,
actions and custom options. Below you will find the tutorial that leads
through the process of creating a plugin.
...
in netadmin.plugins). This subclass will be a starting point for all plugin's features.
...
NOTE: Writing a comprehensive docstring would be a great advantage (it is
...
NOTE: In fact every option is stored as a string--its value is converted to string
...
this object, for example, to access user who ownes the widget (for more
...
events etc. To read more about shortcuts see documentation of netadmin.shortcuts
module.

NOTE: Shortcut functions were made also for backward compatibility, so you
